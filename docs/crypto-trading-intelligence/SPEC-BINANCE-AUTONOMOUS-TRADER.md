# SPEC — Binance Bot Autonomous Trader v1.7

**Date locked:** 2026-06-19 (v1.7 — Phase 11C governance cross-links appended; v1.6 was Phase 11A execution-layer guard)
**Owner:** BossMan Hermes
**Status:** Phase 2-4 complete (Stage 6 + Stage 7 wired; L-CRYPTO-14/15/16/17 active; Phase 11A execution-layer $75 guard live; Phase 11C governance pointers appended; PM2 online port 8104)
...
**v1.7 deltas (2026-06-19, Phase 11C — governance-only):**
- NEW subsection "Upstream governance specs (Phase 11C, 2026-06-19)" right under the Operator-floors table. References `SPEC-STRATEGY-CLASSES-AGGRESSION-TIERS.md` v0.1 (L-CRYPTO-18) and `SPEC-MARKET-REGIMES.md` v0.1 (L-CRYPTO-19) as upstream governance pointer specs.
- Cross-reference: governance specs define vocabulary; this SPEC does NOT adopt their numeric bands as engine constants.
- LEARNED rule count: 17 → 19 (L-CRYPTO-18 and L-CRYPTO-19 added; both pure spec pointers).
- **No other section changed.** No operator floor, schema field, sizing math, numeric risk band, runtime constant, PM2 process, .env, or cron changed. The $75 floor (L-CRYPTO-14, L-CRYPTO-17), Stage 6 emitter (L-CRYPTO-15), and Stage 7 reader (L-CRYPTO-16) remain frozen.
...
### Upstream governance specs (Phase 11C, 2026-06-19) — referenced, NOT enforced here

The following governance specs define the vocabulary BossMan uses when filling `per_coin[sym].strategy_class`, `per_coin[sym].aggression_tier`, and `metadata.regime_today` in `data/bossman_decision.json`. They are **upstream references only** for this SPEC; this SPEC does not adopt their numeric bands as engine constants.

- `~/.hermes/knowledge/SPEC-STRATEGY-CLASSES-AGGRESSION-TIERS.md` v0.1 (L-CRYPTO-18) — strategy classes (scalper/swing/position/hedge) and aggression tiers (TIER_1_CONSERVATIVE/TIER_2_BASE/TIER_3_AGGRESSIVE). Numeric bands are **governance guardrails and typical ranges**, not engine constants. Implementation-level constants in this SPEC and in the bot code may be **narrower**; they must NOT exceed the L-CRYPTO-18 ceilings without a new L-CRYPTO rule.
- `~/.hermes/knowledge/SPEC-MARKET-REGIMES.md` v0.1 (L-CRYPTO-19) — 7-value market-regime vocabulary (EARLY_CYCLE, MID_CYCLE, LATE_CYCLE, DISTRIBUTION, RISK_OFF, RECOVERY, UNKNOWN). The enum is canonical per L-CRYPTO-15 §6; this spec defines each value.

**Status:** Phase 11C pass is **governance-only**. No operator floor, schema field, sizing math, numeric risk band, or runtime constant in this SPEC changed. The $75 floor (L-CRYPTO-14, L-CRYPTO-17), Stage 6 emitter (L-CRYPTO-15), and Stage 7 reader (L-CRYPTO-16) remain frozen.
