...
> **⚠️ SUPERSEDED (2026-05-27):** See `HERMES_MASTER_BLUEPRINT.md` in Hermes knowledge for **AI Stack v2** — full layer definitions, cost tiers, routing policy, knowledge reuse policy, and Ollama integration. This section is historical only.

||| Model | Role | When to Use |
|-------|------|------------|
| **BossMan / Hermes** | **ONLY orchestration authority** | All routing decisions — never bypassed |
| **Perplexity Search** | Research plane | Live web research, current facts, citations |
| **Perplexity Computer** | Action plane | Multi-step browser workflows, connectors (credits) |
| **Ollama / Llama** | Local execution | Summaries, drafts, extraction, first-pass code (zero cost) |
| **MiniMax 2.7** | General cloud | Routine orchestration, standard execution |
...
## Client Review Portal + Helpdesk Ticket System

**Permanent blueprint:** `CLIENT_REVIEW_PORTAL_HELPDESK_BLUEPRINT.md`

### Architecture
- ONE internal command center (Kanban = source of truth, BossMan = operator)
- ONE authenticated customer portal (account-based auth + project-level authorization)
- TWO customer-facing service modes:
  - **Client Review Portal** — pre-launch structured customer testing/review
  - **Helpdesk Ticket System** — post-launch ongoing support (monthly/yearly customers)

### Access Model
Each customer gets credentials scoped to their own project only. No shared passwords. No cross-customer visibility.

### Approval Rules (Permanent)
Marcelo is pulled in ONLY for: major scope/architecture/budget changes, security or system-level risks, real external vendor/account blockers that cannot be resolved locally.

BossMan handles autonomously: routine troubleshooting, customer follow-up, ticket triage and close loops, FAQ matching, auto-follow-up scheduling, internal tool updates.

### Brain-Layer Policy (Permanent — Reusable Across All Blueprints)

> Copy/paste intact into any project blueprint. Project-agnostic.
...
**Marcelo** = approval layer only. Pulled in only for: true external vendor/account blockers, major product or security changes, design decisions with visible customer impact. Marcelo is NOT a copy/paste relay, daily operator, or information shuttle.

### Direct BossMan ↔ Perplexity Loop (No Manual Copy/Paste) — Permanent

For any significant investigation, design, or troubleshooting task:
...
**Standard pattern:** `BossMan ↔ Perplexity ↔ Hermes` with Marcelo at approval layer only — NOT a clipboard. Marcelo must NOT be used as a human clipboard between Perplexity and Hermes.
...
### See Also
- Full blueprint: `CLIENT_REVIEW_PORTAL_HELPDESK_BLUEPRINT.md`
- Brain-layer + Perplexity policy: same blueprint, Brain-Layer section
- Retire dashboards
