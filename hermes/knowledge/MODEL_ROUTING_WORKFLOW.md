...
| 3.1 | 2026-06-18 | **Phase 4 — Loop Protocol (§10).** Added §10.1–10.6. Strict `project:` tag (closed vocabulary), mandatory Tier-4/5 reuse-check via `ARTIFACT_INDEX.md`. **§1–9 unchanged.** |
| 3.2 | 2026-06-18 | **§10.7 — Per-project concurrency cap (dispatcher pick-time rule).** Added model-routing perspective; full canonical definition in `ROUTING-RULES.md` §10.6. §1–9 unchanged. |
...
## 10. Phase 4 — Loop Protocol (additive)

**Status:** ACTIVE 2026-06-18. §1–9 above are unchanged.

Phase 4 turns the §6 "Paid Model Artifact Reuse Policy" from a written
rule into a routing-ledger field. Before every Tier-4/5 model call
(DeepSeek / OpenAI / Claude / Perplexity Computer), the caller MUST
execute a pre-flight reuse-check. After every such call, the caller MUST
save the output to `~/.hermes/knowledge/` and add a row to
`ARTIFACT_INDEX.md`.

### 10.1 Updated Routing Ledger template (additive)

The Routing Ledger gains two new fields, placed AFTER `next_model_planned`
and BEFORE any free-form notes. Existing fields unchanged.

```yaml
# Routing Ledger — Phase 4 extended template
project: <tag from PROJECT_VOCABULARY.md>
work_type: new_build | existing_build | troubleshooting
lead_model: <model>
...
cost_tier: <1-5>
qa_required: yes | no
qa_model: <model>          # if qa_required: yes
qa_status: pending | passed | failed
escalate_to_computer: yes | no
build_passes: yes | no
rewrite_scope: yes | no
last_model_used: <model>
next_model_planned: <model|none>
reuse_check: yes | no                  # NEW — Phase 4
artifact_index_entry: <path>           # NEW — Phase 4
```

`reuse_check: yes` means the call was SKIPPED (artifact reused).
`reuse_check: no` means the call ran and produced a new artifact saved to
the path in `artifact_index_entry:`.

### 10.2 Pre-flight reuse-check (loop hook)

Every Tier-4/5 call site must run this BEFORE invoking the model:

```bash
# 1. Build a 1-3 keyword summary of the analysis
KEYWORDS="<keyword1> <keyword2> <keyword3>"

# 2. Grep the index
HIT=$(grep -i "$KEYWORDS" ~/.hermes/knowledge/ARTIFACT_INDEX.md | head -1)

if [ -n "$HIT" ]; then
  # Reuse — extract filename from the row
  ARTIFACT=$(echo "$HIT" | awk -F'|' '{print $5}' | tr -d '` ')
  echo "REUSE: $ARTIFACT"
  # Ledger field: reuse_check: yes | reused: <full path>
else
  # Proceed with call; save output to ~/.hermes/knowledge/ afterward
  echo "PROCEED: no matching artifact in index"
  # Ledger field: reuse_check: no | artifact_index_entry: <new path>
fi
```

### 10.3 Valid skip reasons

The reuse-check may be skipped ONLY when:

- `reuse_check: skip | reason: index_unavailable` — `ARTIFACT_INDEX.md`
  cannot be read (filesystem error, permissions, etc.)
- `reuse_check: override | reason: <text>` — Marcelo explicitly requested a
  fresh analysis ("regenerate the Binance SPEC from scratch")

Any other skip must be logged as a violation. BossMan reviews ledger
violations weekly.

### 10.4 Post-call save protocol

When `reuse_check: no`, the caller:

1. Saves the full model output to `~/.hermes/knowledge/<descriptive-name>.md`
2. Adds ONE row to `ARTIFACT_INDEX.md` with:
   - `Date`: today (`YYYY-MM-DD`)
   - `Project`: closed-vocabulary tag
   - `Tier`: `T4` or `T5`
   - `Filename`: exact filename
   - `One-line summary`: ≤ 100 chars
3. Logs `artifact_index_entry: ~/.hermes/knowledge/<filename>` in the
   Routing Ledger
...
```yaml
reuse_check: yes
artifact_index_entry: ~/.hermes/knowledge/SPEC-BINANCE-AUTONOMOUS-TRADER.md
last_model_used: spec-claude (reused 2026-06-18 spec)
next_model_planned: m3
```
...
```yaml
reuse_check: no
artifact_index_entry: ~/.hermes/knowledge/PHASE_4_MULTI_PROJECT_OS.md
last_model_used: claude-opus-4-7
next_model_planned: deepseek-r1
```

**Skip with override:**

```yaml
reuse_check: override
reuse_reason: marcelo_override:"regenerate Binance SPEC from scratch, ignore v1.2"
artifact_index_entry: ~/.hermes/knowledge/SPEC-BINANCE-AUTONOMOUS-TRADER-v2.md
last_model_used: claude-opus-4-7
next_model_planned: deepseek-r1 (QA)
```

### 10.6 Enforcement

- BossMan reviews the ledger at week boundary.
- Cards with `reuse_check: skip` or `reuse_check: override` are
  surfaced in the weekly review.
- Cards with `reuse_check` missing on Tier-4/5 calls are surfaced as
  violations.

### 10.7 Per-project concurrency cap (dispatcher pick-time rule)

**Effective:** 2026-06-18 (Phase 4 v3.2). Section 10.2 mechanism
revised; see `ROUTING-RULES.md` §10.6 for the canonical definition.

This section documents the rule from the model-routing / operator-
workflow angle. The routing angle:

- **When is the cap applied?** At dispatcher pick time, inside
  `dispatch_once()`. The cap is NOT a model-routing rule — it does NOT
  influence which model is selected, only whether the dispatch is
  attempted at all on this tick.
- **What happens to a deferred task?** It stays `ready`. The next
  dispatch tick re-evaluates. No retry counter, no exponential
  backoff, no escalation. Pure FIFO via `ORDER BY priority DESC,
  created_at ASC`.
- **Worked example.** Suppose `kanban.max_in_progress_per_project=2`,
  `binance-bot` project has 4 ready tasks (A, B, C, D by priority),
  and 1 already running:
  - Tick 1: A and B are dispatched (count=1 running + 2 picked = 3
    visible — wait, that's > N=2). **Correction:** count is
    `running_count + picked_this_tick ≤ N`. With 1 running + N=2,
    only 1 picked this tick. So: A picked, B/C/D deferred.
  - Tick 2 (A still running): B picked, C/D deferred.
  - Tick 3 (A finished): running_count=0, so 2 picked: B and C.
  - Tick 4 (B/C finished): D picked.
- **Interaction with model routing.** None. The cap sits in the
  dispatcher pre-spawn path; the model choice in §3 / §4 fires only
  after spawn. A deferred task is not "given a different model" —
  it's not spawned at all.
- **Routing Ledger impact.** None. No new ledger fields. The cap is
  observable via `DispatchResult.skipped_per_project_capped` if you
  want to surface it in a custom telemetry pipeline.

See `ROUTING-RULES.md` §10.6 for config, implementation surface,
and the cap-vs-recompute invariant.
