...
> **SECURITY & PM2 WATCH GOAL LOOP (Phase S1, registered 2026-06-23):** Monthly Goal Loop that wraps security-watch daily/weekly and pm2-health-check. Goal card t_e56d53cd, loop spec at ~/.hermes/knowledge/GOAL-LOOP-SECURITY_PM2.md, cron driver at ~/.hermes/scripts/security-pm2-monthly.sh (no-agent; schedule 30 23 1 * *; no-spam default = local-only; Telegram only on P0 or Step-5 FAIL). Five-step loop: INTAKE → DECOMPOSE → EXECUTE → REVIEW → DONE. Hard STOPs: no PM2 deletes, no port opens/closes, no service restarts, no kernel-doc edits. P1+ findings must produce separate fix cards; the loop only surfaces drift. Step-5 QA is mandatory (M3 + DeepSeek per routing), and S1.202606 first cycle PASS is recorded (commit `02c8851`).
